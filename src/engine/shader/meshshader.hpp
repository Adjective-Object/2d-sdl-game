...
#define MESH_SHADER_ATTRIBUTES \
    ...
    attribute(boneCount) \
    attribute(inBoneIndex) \
    attribute(inBoneWeights) \

#define MESH_SHADER_UNIFORMS \
    uniform(baseTransform) \
    uniform(ambientTexture) \
    uniform(boneMatrixArray) \
...
#define attribute(name) name = (offsetof(MeshShaderAttributes, name) / sizeof(GLint)),
#define uniform(name) name = (offsetof(MeshShaderUniforms, name) / sizeof(GLint)),

enum MESH_SHADER_UNIFORM {
    MESH_SHADER_UNIFORMS
    __NUM_UNIFORMS
};

enum MESH_SHADER_ATTRIBUTE {
    MESH_SHADER_ATTRIBUTES
    __NUM_ATTRIBUTES
};
